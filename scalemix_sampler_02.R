...
scalemix.sampler.02 <- function(Y, cen, thresh,
                                initial.values,
                                n.updates, thin=10,
  ...
  for (i in 2:n.updates) {
    ...
    X.s.res<-X.s.update.mixture.me.update.par.once.without.X.par(R=R, 
                    ...
                    theta.gpd=theta.gpd, delta=delta,tau_sqd=tau, thresh.X=NULL)
    X.s <- X.s.res$X.s
...
#                    gamma1*(r.hat.prob.below - metr.opt.1d))
    ...
    if ((i %% echo.interval) == 0) {
      ...
      # Sys.sleep(0.1)
      state <- list(Y=Y, cen=cen, thresh=thresh,
                    experiment.name="Huser-wadsworth",
                    i=i, sigma.m=sigma.m, prop.Sigma=prop.Sigma, 
                    X=X, X.s=X.s, theta.gpd=theta.gpd, prob.below=prob.below,
                    delta=delta, R=R, tau=tau)
      out.obj <- list(Y=Y, cen=cen, thresh=thresh,
                      i=i,
                      X.s.trace=X.s.trace,
                      X.trace=X.trace,
