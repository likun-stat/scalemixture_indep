...
// [[Rcpp::export()]]
double X_s_likelihood_conditional_uni(double X_s, double R){
    
    double part2 = log(R)-2*log(X_s);
    if(X_s<R) {return -std::numeric_limits<double>::infinity();}
    else{return part2;}
}
...
List update_X_s_onetime (NumericVector Y, NumericVector X, NumericVector X_s,
    ...
    for(int iter=0; iter<n_s; iter++){
        ...
        prop_X_s(iter) = temp;
        log_num = marg_transform_data_mixture_me_likelihood_uni(Y(iter), X(iter), prop_X_s(iter), cen(iter), prob_below, theta_gpd, delta, tau_sqd, thresh_X) + X_s_likelihood_conditional_uni(prop_X_s(iter), R);
        log_denom = marg_transform_data_mixture_me_likelihood_uni(Y(iter), X(iter), X_s(iter), cen(iter), prob_below, theta_gpd, delta, tau_sqd, thresh_X) + X_s_likelihood_conditional_uni(X_s(iter), R);
